    ...
    try {
      const res = await axios.post(
        `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/auth/signup`,
        {
          ...
          adminCode: adminMode ? adminCode : undefined,
        }
      );
      ...
      if (res.data.message.toLowerCase().includes("success")) {
        const isAdmin = res.data?.user?.role === 'admin' || adminMode;
        setTimeout(() => {
          navigate("/login", { state: isAdmin ? { msg: "Admin account created. Please login." } : undefined });
        }, 1200);
      }
    } catch (err) {
      ...
      errorMessage.style.cssText = `
        ...
        animation: slideInRight 0.3s ease-out;
      `;
      errorMessage.textContent = err.response?.data?.message || "Signup failed";
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: "var(--spacing-2xl)" }}>
          ...
          <p style={{
            ...
            Join PetCare and give your pets the care they deserve
          </p>
        </div>
        ...
        <div style={{ marginBottom: "var(--spacing-xl)" }}>
          <div style={{ 
            ...
            <input
              ...
              id="addPet"
              checked={showPetForm}
              onChange={(e) => setShowPetForm(e.target.checked)}
              style={{ 
                transform: "scale(1.3)",
                accentColor: "var(--primary-navy)"
              }}
            />
            <label htmlFor="addPet" style={{ 
              fontFamily: "var(--font-heading)",
              color: "var(--primary-navy)", 
              fontSize: "1.1rem",
              fontWeight: "600",
